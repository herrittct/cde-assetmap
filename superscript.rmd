...
```{r geometry, eval=FALSE, include=FALSE}
...
```{r meal_count, eval=FALSE, include=FALSE}
...
```{r meal count max, include=FALSE}
...
rm("import_meal_count", "meal_count_column_unknown",
   "meal_count_directory", "count_files", "huge_meal_count")
```
...
district_meal_count <- full_join(district_meal_count, sfsp_sheets)
```
...
```{r graph_meal_count, eval=FALSE, include=FALSE}
 district_meal_count %>% 
...
```{r refining meal counts, include=FALSE}
...
```{r District ADP by Month, eval=FALSE, include=FALSE}
...
working_student_count <- group_by(raw_student_count, 
                                  sfa_num, school_year_num, site_num) %>% 
  summarize(pk_12_enrollment_total = mean(pk_12_enrollment),
            free_perc = mean(free_perc),
            redu_perc = mean(redu_perc)) %>% 
  group_by(sfa_num, school_year_num) %>% 
  summarize(pk_12_enrollment_total = sum(pk_12_enrollment_total))
...
```{r combine components, include=FALSE}

combined_sfsp_geodist <- combined_geo_dist
...
```{r score calculation, include=FALSE}
...
final_district_dataframe <- mutate(final_district_dataframe, 
      ...
      adp_score = adp_average / pk_12_enrollment_total)

final_district_dataframe <- mutate(final_district_dataframe, 
                                   score_sfsp_sites = case_when(
  !is.na(site_count_sfsp) ~c(site_count_sfsp / count_schools),
  is.na(site_count_sfsp) ~0), 
  score = score_4day + district_size_score + score_urban_rural+ score_cep + score_sfsp_sites + free_perc.y + redu_perc.y + adp_score) 
...
```{r sfsp maps, include=FALSE}

mapping_data <- select(final_district_dataframe,
                       sfa_name, 
                       free_and_red_perc,
                       meal_type, 
                       site_count_sfsp,
...
mapping_data_popups <- paste0("District Name: ", mapping_data$sfa_name, "<br>",
                                       "Free and Reduced Percent: ", round(mapping_data$free_and_red_perc, digits = 3), "<br>",
                                       #"ADP Average: ", round(mapping_data$adp_average, digits = 3), "<br>",
                                       "Program Scores: ", mapping_data$meal_type, "<br>",
                                       "SFSP Site Count: ", mapping_data$site_count_sfsp, "<br>",
                                       "SFSP Average ADP: ", mapping_data$adp_sfsp_2018)

sfsp_pal <- colorNumeric(palette = "plasma", 
                                domain = mapping_data$adp_sfsp_2018)
# sfsp_map <- 
...
mapping_data %>% 
  ...
  addPolygons(popup = mapping_data_popups,
              stroke = FALSE,
              smoothFactor = 0,
              fillOpacity = 0.6,
              color = ~ sfsp_pal(adp_sfsp_2018),
              highlightOptions = highlightOptions(color = "white", weight = 2,
                                                  bringToFront = TRUE)) %>%
  addLegend("bottomright", 
            pal = sfsp_pal, 
            values = ~ adp_sfsp_2018,
            title = "adp_sfsp_2018",
            opacity = 1)

#htmlwidgets::saveWidget(sfsp_map, file="sfsp.html", selfcontained= TRUE)
...
```{r maps with layers instead of multiple maps}
...
Build_popups <- function(x, df){
  popup_built <- list() %>% 
    popup_built <- paste0(names(x), df[[x]], "<br>")}

list_test <- list("District Name: " = "sfa_name",
                  "Score: " = "score")

#Build_popups(x = list_test, df = final_district_dataframe)

final_district_dataframe_popups_score <- paste0(
  "District Name: ",
  final_district_dataframe[["sfa_name"]],  "<br>",
  "Free and Reduced Percent: ", 
  round(final_district_dataframe[["free_and_red_perc"]], digits = 3), "<br>",
  "Score: ", 
  round(final_district_dataframe[["score"]], digits = 3), "<br>", 
  "School Breakfast Program: ",
  final_district_dataframe[["sbp"]], "<br>",
  "National School Lunch Program: ", 
  final_district_dataframe[["nslp"]], "<br>",
  "Afterschool Snack Program: ", 
  final_district_dataframe[["snack"]], "<br>",
  "Special Milk Program: ", 
  final_district_dataframe[["smp"]])
...
final_district_dataframe %>% 
  ...
  addLegend("bottomright",
            ...
            title = "Summer Food Service Program Average ADP 2017",
            opacity = 1,
            ) %>%
  addLayersControl(
...
plot_all_score <- function(df, x){
  tigris::plot(df[x])
}



plot_all_score(scores_final, 4
              )

               tigris::plot(scores_final, max.plot =41)
```
